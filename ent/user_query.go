...
// QueryFavorite chains the current query on the "favorite" edge.
func (uq *UserQuery) QueryFavorite() *ArticleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, user.FavoriteTable, user.FavoritePrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryMyComments chains the current query on the "myComments" edge.
func (uq *UserQuery) QueryMyComments() *CommentQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.MyCommentsTable, user.MyCommentsColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withFavorite:   uq.withFavorite.Clone(),
		withMyComments: uq.withMyComments.Clone(),
...
// WithFavorite tells the query-builder to eager-load the nodes that are connected to
// the "favorite" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithFavorite(opts ...func(*ArticleQuery)) *UserQuery {
	query := &ArticleQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withFavorite = query
	return uq
}

// WithMyComments tells the query-builder to eager-load the nodes that are connected to
// the "myComments" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithMyComments(opts ...func(*CommentQuery)) *UserQuery {
	query := &CommentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withMyComments = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withFavorite; query != nil {
		edgeids := make([]driver.Value, len(nodes))
		byid := make(map[int]*User)
		nids := make(map[int]map[*User]struct{})
		for i, node := range nodes {
			edgeids[i] = node.ID
			byid[node.ID] = node
			node.Edges.Favorite = []*Article{}
		}
		query.Where(func(s *sql.Selector) {
			joinT := sql.Table(user.FavoriteTable)
			s.Join(joinT).On(s.C(article.FieldID), joinT.C(user.FavoritePrimaryKey[0]))
			s.Where(sql.InValues(joinT.C(user.FavoritePrimaryKey[1]), edgeids...))
			columns := s.SelectedColumns()
			s.Select(joinT.C(user.FavoritePrimaryKey[1]))
			s.AppendSelect(columns...)
			s.SetDistinct(false)
		})
		neighbors, err := query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
		...
		for _, n := range neighbors {
			nodes, ok := nids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "favorite" node returned %v`, n.ID)
			}
			for kn := range nodes {
				kn.Edges.Favorite = append(kn.Edges.Favorite, n)
			}
		}
	}

	if query := uq.withMyComments; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.MyComments = []*Comment{}
		}
		query.withFKs = true
		query.Where(predicate.Comment(func(s *sql.Selector) {
			s.Where(sql.InValues(user.MyCommentsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_my_comments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_my_comments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_my_comments" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.MyComments = append(node.Edges.MyComments, n)
		}
	}
