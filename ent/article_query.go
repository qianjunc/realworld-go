...
// QueryFavorited chains the current query on the "favorited" edge.
func (aq *ArticleQuery) QueryFavorited() *UserQuery {
	query := &UserQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(article.Table, article.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, article.FavoritedTable, article.FavoritedPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(aq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryArticleComments chains the current query on the "articleComments" edge.
...
func (aq *ArticleQuery) Clone() *ArticleQuery {
	...
	return &ArticleQuery{
		...
		withFavorited:       aq.withFavorited.Clone(),
...
// WithFavorited tells the query-builder to eager-load the nodes that are connected to
// the "favorited" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *ArticleQuery) WithFavorited(opts ...func(*UserQuery)) *ArticleQuery {
	query := &UserQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withFavorited = query
	return aq
}

// WithArticleComments tells the query-builder to eager-load the nodes that are connected to
// the "articleComments" edge. The optional arguments are used to configure the query builder of the edge.
...
func (aq *ArticleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Article, error) {
	...
	if query := aq.withArticleAuthor; query != nil {
		...
		for _, n := range neighbors {
			...
			for i := range nodes {
				nodes[i].Edges.ArticleAuthor = n
			}
		}
	}

	if query := aq.withFavorited; query != nil {
		...
		for i, node := range nodes {
			edgeids[i] = node.ID
			byid[node.ID] = node
			node.Edges.Favorited = []*User{}
		}
		query.Where(func(s *sql.Selector) {
			joinT := sql.Table(article.FavoritedTable)
			s.Join(joinT).On(s.C(user.FieldID), joinT.C(article.FavoritedPrimaryKey[1]))
			s.Where(sql.InValues(joinT.C(article.FavoritedPrimaryKey[0]), edgeids...))
			columns := s.SelectedColumns()
			s.Select(joinT.C(article.FavoritedPrimaryKey[0]))
		...
		for _, n := range neighbors {
			nodes, ok := nids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "favorited" node returned %v`, n.ID)
			}
			for kn := range nodes {
				kn.Edges.Favorited = append(kn.Edges.Favorited, n)
			}
		}
	}
	...
	if query := aq.withArticleComments; query != nil {
		...
		for _, n := range neighbors {
			fk := n.article_article_comments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "article_article_comments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "article_article_comments" returned %v for node %v`, *fk, n.ID)
			}
