...
func (uc *UserCreate) AddFavorite(a ...*Article) *UserCreate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
func (uc *UserCreate) AddMyCommentIDs(ids ...int) *UserCreate {
	uc.mutation.AddMyCommentIDs(ids...)
	return uc
}

// AddMyComments adds the "myComments" edges to the Comment entity.
func (uc *UserCreate) AddMyComments(c ...*Comment) *UserCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return uc.AddMyCommentIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.FavoriteIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
	...
	if nodes := uc.mutation.MyCommentsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
