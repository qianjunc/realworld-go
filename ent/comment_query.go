...
// QueryCommentArticle chains the current query on the "commentArticle" edge.
func (cq *CommentQuery) QueryCommentArticle() *ArticleQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, comment.CommentArticleTable, comment.CommentArticleColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withCommentArticle: cq.withCommentArticle.Clone(),
...
// WithCommentArticle tells the query-builder to eager-load the nodes that are connected to
// the "commentArticle" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommentQuery) WithCommentArticle(opts ...func(*ArticleQuery)) *CommentQuery {
	query := &ArticleQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withCommentArticle = query
	return cq
}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	var (
		...
		loadedTypes = [2]bool{
			...
			cq.withCommentArticle != nil,
		}
	)
	if cq.withCommentAuthor != nil || cq.withCommentArticle != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withCommentAuthor; query != nil {
		...
		for i := range nodes {
			if nodes[i].user_my_comments == nil {
				continue
			}
			fk := *nodes[i].user_my_comments
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_my_comments" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.CommentAuthor = n
			}
		}
	}

	if query := cq.withCommentArticle; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Comment)
		for i := range nodes {
			if nodes[i].article_article_comments == nil {
				continue
			}
			fk := *nodes[i].article_article_comments
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "article_article_comments" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.CommentArticle = n
			}
		}
	}
