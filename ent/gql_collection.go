...
func (a *ArticleQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		...
		case "favorited":
			...
			a.withFavorited = query
		case "articlecomments", "articleComments":
			var (
				path  = append(path, field.Name)
...
func (c *CommentQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		switch field.Name {
		case "commentauthor", "commentAuthor":
			var (
				path  = append(path, field.Name)
			...
			c.withCommentAuthor = query
		case "commentarticle", "commentArticle":
			var (
				path  = append(path, field.Name)
...
func (t *TagQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		switch field.Name {
		case "tagarticle", "tagArticle":
			var (
				path  = append(path, field.Name)
				query = &ArticleQuery{config: t.config}
			)
			if err := query.collectField(ctx, op, field, path, satisfies...); err != nil {
				return err
			}
			t.withTagArticle = query
		}
	}
	return nil
}
...
func (u *UserQuery) collectField(ctx context.Context, op *graphql.OperationContext, field graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(op, field.Selections, satisfies) {
		...
		case "favorite":
			...
			u.withFavorite = query
		case "mycomments", "myComments":
			var (
				path  = append(path, field.Name)
